...
def test_bytewise():
    ...
    common(BitStruct(
        "p1" / Nibble,
        "num" / Bytewise(Int24ub),
        "p2" / Nibble
    ), b"\xf0\x10\x20\x3f", Container(p1=15, num=0x010203, p2=15), 4)
...
def test_bytesinteger():
    ...
    with pytest.raises(IntegerError):
        BytesInteger(-1).parse(b"")
    with pytest.raises(IntegerError):
        BytesInteger(-1).build(0)
    with pytest.raises(IntegerError):
        BytesInteger(8).build(None)
    with pytest.raises(IntegerError):
        BytesInteger(8, signed=False).build(-1)
    with pytest.raises(IntegerError):
        BytesInteger(8, True).build(-2**64)
    with pytest.raises(IntegerError):
        BytesInteger(8, True).build(2**64)
    with pytest.raises(IntegerError):
        BytesInteger(8, False).build(-2**64)
    with pytest.raises(IntegerError):
        BytesInteger(8, False).build(2**64)
    with pytest.raises(SizeofError):
...
def test_bitsinteger():
    ...
    with pytest.raises(IntegerError):
        BitsInteger(-1).parse(b"")
    with pytest.raises(IntegerError):
        BitsInteger(-1).build(0)
    with pytest.raises(IntegerError):
        BitsInteger(5, swapped=True).parse(bytes(5))
    with pytest.raises(IntegerError):
        BitsInteger(5, swapped=True).build(0)
    with pytest.raises(IntegerError):
        BitsInteger(8).build(None)
    with pytest.raises(IntegerError):
        BitsInteger(8, signed=False).build(-1)
    with pytest.raises(IntegerError):
        BitsInteger(8, True).build(-2**64)
    with pytest.raises(IntegerError):
        BitsInteger(8, True).build(2**64)
    with pytest.raises(IntegerError):
        BitsInteger(8, False).build(-2**64)
    with pytest.raises(IntegerError):
        BitsInteger(8, False).build(2**64)
    with pytest.raises(SizeofError):
...
def test_encryptedsym():
    ...
    with pytest.raises(CipherError):
        EncryptedSym(GreedyBytes, "AES").build(b"")
    with pytest.raises(CipherError):
        EncryptedSym(GreedyBytes, "AES").parse(b"")
...
def test_encryptedsymaead():
    ...
    with pytest.raises(CipherError):
        EncryptedSymAead(GreedyBytes, "AESGCM", bytes(16)).build(b"")
    with pytest.raises(CipherError):
        EncryptedSymAead(GreedyBytes, "AESGCM", bytes(16)).parse(b"")
